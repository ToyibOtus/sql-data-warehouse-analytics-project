...
BEGIN
	...
	@rows_loaded INT,
	@file_path NVARCHAR(MAX) = 'C:\Users\PC\Documents\SQL_DataWareHouseProject\sql-data-warehouse-project\datasets\source_crm\prd_info.csv',
	@sql NVARCHAR(MAX);
	...
	BEGIN TRY
		...
		SET @sql = 'BULK INSERT landing.crm_prd_info FROM ''' + @file_path + ''' WITH (FIRST_ROW = 2, FIELDTERMINATOR = '','', TABLOCK);';
		-- Load data to table
		EXEC (@sql);
