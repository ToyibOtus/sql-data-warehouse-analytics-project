...
=====================================================================================================
Script Purpose:
	This script loads [landing.erp_px_cat_g1v2]. It also performs logging operations by loading vital
	log details into log tables [etl_step_run] & [etl_error_log].

Parameter: @job_run_id UNIQUEIDENTIFIER = NULL

Usage: EXEC landing.usp_load_landing_erp_px_cat_g1v2;
...
=====================================================================================================
*/
CREATE OR ALTER PROCEDURE landing.usp_load_landing_erp_px_cat_g1v2 @job_run_id UNIQUEIDENTIFIER = NULL AS
BEGIN
	...
	@step_name NVARCHAR(50) = 'usp_load_landing_erp_px_cat_g1v2',
	@start_time DATETIME,
	@end_time DATETIME,
	@step_duration INT,
	@step_status NVARCHAR(50) = 'RUNNING',
	@rows_loaded INT,
	@source_path NVARCHAR(MAX) = 'C:\Users\PC\Documents\SQL_DataWareHouseProject\sql-data-warehouse-project\datasets\source_erp\px_cat_g1v2.csv',
	@sql NVARCHAR(MAX);
	...
	BEGIN TRY
		...
		SET @sql = 'BULK INSERT landing.erp_px_cat_g1v2 FROM ''' + @source_path + ''' WITH (FIRST_ROW = 2, FIELDTERMINATOR = '','', TABLOCK);';
		-- Load data to table
		EXEC (@sql);
		...
		SELECT @rows_loaded = COUNT(*) FROM landing.erp_px_cat_g1v2;
