...
=====================================================================================================
Script Purpose:
	This script loads [landing.crm_sales_details]. It also performs logging operations by loading vital
...
=====================================================================================================
*/
CREATE OR ALTER PROCEDURE landing.usp_load_landing_crm_sales_details @job_run_id UNIQUEIDENTIFIER = NULL AS
BEGIN
	...
	@step_name NVARCHAR(50) = 'usp_load_landing_crm_sales_details',
	@start_time DATETIME,
	@end_time DATETIME,
	@step_duration INT,
	@step_status NVARCHAR(50) = 'RUNNING',
	@rows_loaded INT,
	@source_path NVARCHAR(MAX) = 'C:\Users\PC\Documents\SQL_DataWareHouseProject\sql-data-warehouse-project\datasets\source_crm\sales_details.csv',
	@sql NVARCHAR(MAX);
	...
	BEGIN TRY
		...
		SET @sql = 'BULK INSERT landing.crm_sales_details FROM ''' + @source_path + ''' WITH (FIRST_ROW = 2, FIELDTERMINATOR = '','', TABLOCK);';
		-- Load data to table
		EXEC (@sql);
		...
		SELECT @rows_loaded = COUNT(*) FROM landing.crm_sales_details;
