...
BEGIN
	...
	@rows_loaded INT,
	@source_path NVARCHAR(MAX) = 'C:\Users\PC\Documents\SQL_DataWareHouseProject\sql-data-warehouse-project\datasets\source_erp\loc_a101.csv',
	@sql NVARCHAR(MAX);
	...
	BEGIN TRY
		...
		SET @sql = 'BULK INSERT landing.erp_loc_a101 FROM ''' + @source_path + ''' WITH (FIRST_ROW = 2, FIELDTERMINATOR = '','', TABLOCK);';
		-- Load data to table
		EXEC (@sql);
