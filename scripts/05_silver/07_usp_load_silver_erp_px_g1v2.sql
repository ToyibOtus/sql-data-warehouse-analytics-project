...
==================================================================================================================
Script Purpose:
	This script loads the silver table [silver.erp_px_cat_g1v2]. It performs series of data transformations such
...
==================================================================================================================
*/
CREATE OR ALTER PROCEDURE silver.usp_load_silver_erp_px_cat_g1v2 @job_run_id UNIQUEIDENTIFIER = NULL AS
BEGIN
	...
	@step_name NVARCHAR(50) = 'usp_load_silver_erp_px_cat_g1v2',
	@start_time DATETIME,
	@end_time DATETIME,
	@step_duration INT,
	...
	BEGIN TRY
		...
		TRUNCATE TABLE silver_stg.erp_px_cat_g1v2;
		...
		LEFT JOIN silver.erp_px_cat_g1v2 sep
		ON mc.id = sep.id 
		AND mc.dwh_row_hash = sep.dwh_row_hash
		WHERE sep.dwh_row_hash IS NULL;

		-- Retrieve total number of new records from silver_stg
		SELECT @rows_source = COUNT(*) FROM silver_stg.erp_px_cat_g1v2;
		...
		DROP TABLE IF EXISTS #dq_metrics_erp_px_cat_g1v2;

		-- Create and load into temp table vital dq metrics
		SELECT
			COUNT(*) AS rows_checked,
			SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS pk_nulls,
			COUNT(id) - COUNT(DISTINCT id) AS pk_duplicates,
			SUM(CASE WHEN cat != TRIM(cat) THEN 1 ELSE 0 END) AS cat_untrimmed,
			SUM(CASE WHEN cat NOT IN ('Accessories', 'Bikes', 'Clothing', 'Components') THEN 1 ELSE 0 END) AS invalid_cat,
			SUM(CASE WHEN subcat != TRIM(subcat) THEN 1 ELSE 0 END) AS subcat_untrimmed,
			SUM(CASE WHEN maintenance != TRIM(maintenance) THEN 1 ELSE 0 END) AS maintenance_untrimmed,
			SUM(CASE WHEN maintenance NOT IN ('Yes', 'No') THEN 1 ELSE 0 END) AS invalid_maintenance
			INTO #dq_metrics_erp_px_cat_g1v2
		FROM silver_stg.erp_px_cat_g1v2;
		...
		CASE WHEN pk_nulls > 0 THEN 1 ELSE 0 END AS dq_status FROM #dq_metrics_erp_px_cat_g1v2
		UNION ALL
		SELECT @job_run_id, @step_run_id, @layer, @table_name, 'pk_duplicates', rows_checked, pk_duplicates AS rows_failed,
		CASE WHEN pk_duplicates > 0 THEN 1 ELSE 0 END AS dq_status FROM #dq_metrics_erp_px_cat_g1v2
		UNION ALL
		SELECT @job_run_id, @step_run_id, @layer, @table_name, 'cat_untrimmed', rows_checked, cat_untrimmed AS rows_failed,
		CASE WHEN cat_untrimmed > 0 THEN 1 ELSE 0 END AS dq_status FROM #dq_metrics_erp_px_cat_g1v2
		UNION ALL
		SELECT @job_run_id, @step_run_id, @layer, @table_name, 'invalid_cat', rows_checked, invalid_cat AS rows_failed,
		CASE WHEN invalid_cat > 0 THEN 1 ELSE 0 END AS dq_status FROM #dq_metrics_erp_px_cat_g1v2
		UNION ALL
		SELECT @job_run_id, @step_run_id, @layer, @table_name, 'subcat_untrimmed', rows_checked, subcat_untrimmed AS rows_failed,
		CASE WHEN subcat_untrimmed > 0 THEN 1 ELSE 0 END AS dq_status FROM #dq_metrics_erp_px_cat_g1v2
		UNION ALL
		SELECT @job_run_id, @step_run_id, @layer, @table_name, 'maintenance_untrimmed', rows_checked, maintenance_untrimmed AS rows_failed,
		CASE WHEN maintenance_untrimmed > 0 THEN 1 ELSE 0 END AS dq_status FROM #dq_metrics_erp_px_cat_g1v2
		UNION ALL
		SELECT @job_run_id, @step_run_id, @layer, @table_name, 'invalid_maintenance', rows_checked, invalid_maintenance AS rows_failed,
		CASE WHEN invalid_maintenance > 0 THEN 1 ELSE 0 END AS dq_status FROM #dq_metrics_erp_px_cat_g1v2;

		-- Map values to variables
		SELECT @pk_nulls = pk_nulls FROM #dq_metrics_erp_px_cat_g1v2;

		SELECT @pk_duplicates = pk_duplicates FROM #dq_metrics_erp_px_cat_g1v2;
		...
		MERGE silver.erp_px_cat_g1v2 tgt
		USING silver_stg.erp_px_cat_g1v2 src
			ON tgt.id = src.id
